...
import de.ovgu.variantsync.presentationlayer.view.context.ConstraintTextValidator.ValidationMessage;
...
public class ConstraintDialog implements GUIDefaults {
	...
	private IConsumer<ValidationMessage> onCheckStarted = new IConsumer<ValidationMessage>() {
		...
		public void invoke(ValidationMessage message) {
			...
			headerPanel.setDetails(String.format(
					StringTable.CHECK_STARTED,
					(mode == Mode.UPDATE ? StringTable.VERB_UPDATE
							.toLowerCase() : StringTable.VERB_SAVE), okButton
							.getText()),
	...
	private IConsumer<ValidationMessage> onVoidsModelCheckComplete = new IConsumer<ValidationMessage>() {
		...
		public void invoke(ValidationMessage message) {
			if (message.validationResult != ValidationResult.OK) {
				headerPanel.setDetails(StringTable.CONSTRAINT_VOIDS_MODEL,
						HeaderPanel.HeaderDescriptionImage.WARNING);
			}
		}
	};
		...
		public void invoke(ValidationMessage message) {
			if (message.validationResult != ValidationResult.OK) {
				headerPanel.setDetails(
						String.format(StringTable.CONSTRAINT_FALSE_OPTIONAL,
								message.details),
						HeaderPanel.HeaderDescriptionImage.WARNING);
			}
		}
	...
	private IConsumer<ValidationMessage> onDeadFeatureCheckComplete = new IConsumer<ValidationMessage>() {
		...
		public void invoke(ValidationMessage message) {
			if (message.validationResult != ValidationResult.OK) {
				headerPanel.setDetails(String.format(
						StringTable.CONSTRAINT_DEAD_FEATURES, message.details),
						HeaderPanel.HeaderDescriptionImage.WARNING);
			}
		}
	};
	...
	private IConsumer<ValidationMessage> onIsRedundantCheckComplete = new IConsumer<ValidationMessage>() {
		...
		public void invoke(ValidationMessage message) {
			if (message.validationResult != ValidationResult.OK) {
				headerPanel.setDetails(StringTable.CONSTRAINT_REDUNDANCE,
						HeaderPanel.HeaderDescriptionImage.WARNING);
			}
		}
	};
	...
	private IConsumer<ValidationMessage> onCheckEnded = new IConsumer<ValidationMessage>() {
		...
		public void invoke(ValidationMessage message) {
			headerPanel.setDetails(String.format(
					StringTable.CONSTRAINT_CHECK_ENDED,
					(mode == Mode.UPDATE ? StringTable.VERB_UPDATE
							: StringTable.VERB_CREATE),
					(mode == Mode.UPDATE ? StringTable.SAVE_CHANGES
	...
	private IConsumer<ValidationMessage> onIsTautology = new IConsumer<ValidationMessage>() {
		...
		public void invoke(ValidationMessage message) {
			if (message.validationResult != ValidationResult.OK) {
				headerPanel.setDetails(StringTable.CONSTRAINT_TAUTOLOGY,
						HeaderPanel.HeaderDescriptionImage.WARNING);
			}
		}
	};
	...
	private IConsumer<ValidationMessage> onIsNotSatisfiable = new IConsumer<ValidationMessage>() {
		...
		public void invoke(ValidationMessage message) {
			if (message.validationResult != ValidationResult.OK) {
				headerPanel.setDetails(StringTable.CONSTRAINT_NOT_SATISFIABLE,
						HeaderPanel.HeaderDescriptionImage.WARNING);
			}
		}
	};
	 ...
	 * @param constraint
	 */
	public ConstraintDialog(final Constraint constraint) {
		this.constraint = constraint;
		this.featureController = ControllerHandler.getInstance()
				.getFeatureController();

		if (constraint == null) {
			defaultDetailsText = StringTable.DEFAULT_DETAILS_NEW_CONSTRAINT;
			defaultHeaderText = "Feature Expression Dialog";
			initialConstraint = "";
			mode = Mode.CREATE;
	...
	private void autoSetOkButtonText() {
		okButton.setText(String.format(StringTable.OK_BUTTON_TEXT,
				(mode == Mode.UPDATE ? StringTable.VERB_UPDATE
						: StringTable.VERB_CREATE)));
	}
	 ...
	 */
	private void closeButtonPressEvent() {
	 ...
	 * the {@link #closeButtonPressEvent()} method.
	 */
	 ...
	 * @param constraint
	 */
	private void initBottom(final Constraint constraint) {
		GridData gridData = new GridData(GridData.FILL_HORIZONTAL);
		...
		lastComposite.setTabList(new Control[] { okButton, closeButton });
	...
	private void initButtonGroup() {
		...
		for (int i = 0; i < Operator.NAMES.length; i++) {
			...
			button.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
				public void widgetSelected(
						org.eclipse.swt.events.SelectionEvent e) {
					constraintText.copyIn(button.getText().toLowerCase(
							Locale.ENGLISH));
				}
	...
	private void initConstraintText() {
		...
		constraintText = new SimpleSyntaxHighlightEditor(
				constraintTextComposite,
				SWT.SINGLE | SWT.H_SCROLL | SWT.BORDER, Operator.NAMES);
		...
		constraintText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				if (constraintText.getText().trim().isEmpty()) {
					headerPanel.setDetails(
							StringTable.PLEASE_INSERT_CONSTRAINT,
							HeaderPanel.HeaderDescriptionImage.NONE);
					updateDialogState(DialogState.SAVE_CHANGES_DISABLED);
				} else {
					validate();
	...
	private void initFeatureGroup() {
		...
		featureTableViewer.setInput(featureController.getFeatureModel()
				.getFeatures());
	...
	public void setInputText(String text) {
		String constrainText = Operator.isOperatorName(text)
	...
	private void validate() {
		headerPanel.setDetails(StringTable.CHECKING_CONSTRAINTS,
				HeaderPanel.HeaderDescriptionImage.NONE);
		...
		Display.getDefault().asyncExec(new Runnable() {
			...
			@Override
			public void run() {
				ValidationResult result = VALIDATOR.validateSync(
						featureController.getFeatureModel(),
						constraintText.getText());

				constraintText
						.UnderlineEverything(result != ValidationResult.OK
								&& constraintText.getUnknownWords().isEmpty());

				if (result == ValidationResult.OK) {
					VALIDATOR.validateAsync(constraint, VALIDATION_TIME_OUT,
							featureController.getFeatureModel(),
							constraintText.getText(), onCheckStarted,
							onVoidsModelCheckComplete,
							onFalseOptionalCheckComplete,
							onDeadFeatureCheckComplete,
							onIsRedundantCheckComplete, onCheckEnded,
							onIsTautology, onIsNotSatisfiable);
					updateDialogState(DialogState.SAVE_CHANGES_ENABLED);
				} else {
					...
					if (result == ValidationResult.NOT_WELLFORMED) {
						if (constraintText.getUnknownWords().isEmpty()) {
							...
							details = (count == 1 ? StringTable.CONSTRAINT_CONTAINS_UNKNOWN_FEATURE
									: String.format(
											StringTable.CONSTRAINT_CONTAINS_UNKNOWN_FEATURES,
											count));
						}
					} else
						details = "";
					...
					headerPanel.setDetails(String.format(
							StringTable.CONSTRAINT_CONNOT_BE_SAVED, details),

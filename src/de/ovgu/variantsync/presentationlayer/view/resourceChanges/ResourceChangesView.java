...
public class ResourceChangesView extends ViewPart implements AbstractView {
	...
	public ResourceChangesView() {
		...
		controller = VariantSyncPlugin.getDefault().getController();
	}
		...
		doubleClickAction = new Action() {
			public void run() {
				...
				if (obj instanceof ResourceChangesFilePatch) {
					...
					controller.getProjectList(filePatch);
				}
			}
		};
	}
	...
	public void modelPropertyChange(PropertyChangeEvent evt) {
		if (evt.getPropertyName().equals(
			...
			controller.getChanges(selectedfilePatch);
			...
			ProjectSelectionDialog dlg = new ProjectSelectionDialog(viewer
					...
					(String) evt.getNewValue(), controller);
			dlg.setTitle("Project Selection");
			dlg.open();
			Object[] result = dlg.getResult();
			if (result != null && result.length > 0) {
				controller.synchronize(result, selectedfilePatch);
			}
		} else if (evt.getPropertyName().equals(
